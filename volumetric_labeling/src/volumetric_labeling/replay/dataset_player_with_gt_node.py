...
class DatasetPlayer:
  ...
  def __init__(self):
    ...
    self.labeling_type = rospy.get_param("~labeling_type", "uncertainty")
    ...
    labeling_strategy = rospy.get_param("~scoringMethod")
    ...
    self.output_name = f"dataset_{labeling_strategy}_{datetime.datetime.now().strftime('%m_%d_%Y__%H_%M_%S')}"
      ...
      for i in range(1, 40):
        try:
            ...
            if i == 5 or i == 10 or i == 20:
              ...
              map_srv(os.path.join(self.output_folder, self.output_name, f"map_labeled_{i}.panmap"))
