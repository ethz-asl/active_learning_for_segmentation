...
class MinimapManager:
    ...
    def add_pose_to_map(self, msg):
        ...
        if msg.data and self.last_pose is not None:
            if self.current_pose is not None:
                ...
                self.trajectory_plot_map = cv2.line(self.trajectory_plot_map, (left_coord_prev, top_coord_prev), (left_coord, top_coord), (255, 0, 0), thickness=1)
                PILImage.fromarray(self.trajectory_plot_map).save("{}/{}_trajectoryMap.png".format(self.output_path, self.startup_time))
    ...
    def publish_mini_map(self, event):
        ...
        if self.current_pose is not None:
            ...
            #     self.bridge.cv2_to_imgmsg(self.trajectory_plot_map, encoding='rgb8'))
            # Publish Map Image
            self._minimap_pub.publish(
