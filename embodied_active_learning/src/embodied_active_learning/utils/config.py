...
class OnlineLearningConfig(ParsableConfig):
  ...
  rate: float = 0.5
  use_bundle_based_training: bool = False
  bundle_size = 150
  num_epochs = 20
  freeze_bn: bool = True
  ...
  late_start_n_imgs: int = 0

  test_datasets = [("Studio", "/media/rene/Empty/TestSets/Flat/LivingRoomWithDresser")]  # Just as an example
...
class UncertaintyEstimatorConfig(ParsableConfig):
  ...
  THRESHOLD_TYPE_DYNAMIC = "Dynamic"
  THRESHOLD_TYPE_ABSOLUTE = "Absolute"

  type: str = UNCERTAINTY_TYPE_SOFTMAX
  from_logits: str = True
  threshold_type: str = THRESHOLD_TYPE_DYNAMIC
  # If using thresholding
  refitting_rate: int = 1  # Refit after every training iteration
  threshold: int = 0
  quantile: int = 0.9
  max: int = 0
