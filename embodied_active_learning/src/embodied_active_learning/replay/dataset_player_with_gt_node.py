...
from embodied_active_learning.replay.uncertainty import get_uncertainty_estimator_for_config
...
class DatasetPlayer:
  ...
  def __init__(self):
    ...
    self.output_name = f"dataset_{labeling_strategy}_labeled_imgs_{num_images_to_label}_{datetime.datetime.now().strftime('%m_%d_%Y__%H_%M_%S')}_median_filter_10_pc_distance_5"
  ...
  def experiment_end_cb(self, score_name = "scores.csv", create_ds = True):
    ...
    PseudoLabler(config, topic='/render_camera_view').label_many(self.all_samples, cache_time=0)
  ...
  def publish_images(self, timer_event):
    ...
    if self.current_sample_idx == len(self.all_samples):
      ...
      # self.experiment_end_cb()
      self.current_sample_idx = -1
      from embodied_active_learning.srv import label_request
      srv =  rospy.ServiceProxy('/my_service',label_request )
      data = {
        'changed': 0,
        'GTClasses': 0,
        'InstanceClass': 0,
      ...
      import pandas as pd
      self.experiment_end_cb(f"scores_{0}.csv")
      pd.DataFrame.from_dict([data]).to_csv(
        os.path.join(self.output_folder, self.output_name, f"label_information_{0}.csv"), index=False)
      print("DONE DUMPING DATSET")
      print("STARREQUESTING LABELS")
      # srv(10)
      for i in range(1, 40):
        print("Calling service ", i)
        result = srv(30, self.labeling_type)
        print(result)
        print(dir(result))
        data = {
          ...
          'Size': result.Size,
          'Type': self.labeling_type
        }

        import pandas as pd
        self.experiment_end_cb(f"scores_{i}.csv", create_ds = False)
        pd.DataFrame.from_dict([data]).to_csv(os.path.join(self.output_folder, self.output_name, f"label_information_{i}.csv"), index=False)

      print("Done Requesting labels LABELS")
      rospy.signal_shutdown("End of Experiment reached")
      exit()
    ...
    try:
      ...
      if self.current_sample_idx in self.gt_labeled_indexes:
        ...
        uncertainty_msg = self.cv_bridge.cv2_to_imgmsg(1-current_train_sample.weights.astype(np.float32) * 1e5, "32FC1")
        # uncertainty_msg = self.cv_bridge.cv2_to_imgmsg(current_train_sample.uncertainty.astype(np.float32), "32FC1")
